...
export default function ServiceHero({ logoWidth = 180, logoHeight = 80 }) {
  ...
  return (
    <section
      ...
      style={{
        backgroundImage: 'url(/image/pagebg.jpg)',
        backgroundSize: 'cover',
        backgroundPosition: 'center',
      }}
    >
      {/* NAVBAR */}
      <header className="absolute top-0 left-0 w-full z-30 flex items-center justify-center px-4 md:px-16 py-6 md:py-8">
        {/* Logo */}
        ...
        <nav className="hidden md:flex items-center gap-6 lg:gap-8 text-xs sm:text-sm md:text-sm 2xl:text-sm 3xl:text-xl 5xl:text-3xl font-semibold tracking-widest text-white">
          ...
          <a href="/about" className="text-white border-b-2 border-orange-400 pb-1 ">ABOUT</a>
      ...
      >
        ...
        <nav className="flex flex-col items-center text-lg sm:text-xl uppercase tracking-widest text-black space-y-3">
          {["HOME", "ABOUT", "SERVICES", "WORK", "BLOG", "CONTACT"].map((item, i) => (
            <a
              key={i}
              href={`/${item.toLowerCase()}`}
              onClick={() => setMenuOpen(false)}
              className={`py-2 px-6 block transform transition-all duration-500 ${
                menuOpen
      ...
      <div className="flex flex-col items-start justify-center h-full pl-4 sm:pl-8 md:pl-16 pt-36 md:pt-40 5xl:pt-72">
        <h1 className="text-4xl sm:text-5xl md:text-7xl xl:text-6xl 3xl:text-7xl 5xl:text-[10rem] font-extrabold text-white mb-4 sm:mb-6">
          ABOUT
        </h1>
